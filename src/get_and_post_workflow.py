        ...
        if status_code == 201:
            ...
            if response.json() != workflows[f"{workflow_obj['name']} {workflow_obj['version']}"]:
                ...
                logger.info("%s %s %s %s %s",
                    "workflow for name", workflow_obj['name'], "and version", workflow_obj['version'],"is up to date")
        elif status_code == 403:
            logger.info("%s %s %s %s",
                "attempted to update forbidden keys for", workflow_obj['name'], "version", workflow_obj['version'])
        else:
            logger.info("%s %s %s %s",
                "unexpected error in registering workflow for name", workflow_obj['name'], "and version", workflow_obj['version'])
