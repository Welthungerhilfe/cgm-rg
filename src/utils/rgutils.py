...
def upload_to_queue(storage_account_name, json_path, db_connector):
    ...
    with open(json_path) as f:
        ...
        if result_key == 'height':
            front_scan_data = get_measure_insert_dict(
                measure_id, model_id, "front", front_scan_results, height_constant)
            # print(front_scan_data)
            encoded_fcd = str(json.dumps(front_scan_data))
            flag = upload_message(
                queue_service, queue_name, encoded_fcd, flag=0)
            if flag == 0:
                return flag
            back_scan_data = get_measure_insert_dict(
                measure_id, model_id, "back", back_scan_results, height_constant)
            # print(back_scan_data)
            encoded_bcd = str(json.dumps(back_scan_data))
            flag = upload_message(
                queue_service, queue_name, encoded_bcd, flag=0)
            if flag == 0:
                return flag
            threesixty_scan_data = get_measure_insert_dict(
                measure_id, model_id, "360", threesixty_scan_results, height_constant)
            # print(threesixty_scan_data)
            encoded_tcd = str(json.dumps(threesixty_scan_data))
            flag = upload_message(
                queue_service, queue_name, encoded_tcd, flag=0)
            if flag == 0:
                return flag
        elif result_key == 'weight':
            front_scan_data = get_measure_insert_dict(
                measure_id, model_id, "front", front_scan_results, weight_constant)
            # print(front_scan_data)
            encoded_fcd = str(json.dumps(front_scan_data))
            flag = upload_message(
                queue_service, queue_name, encoded_fcd, flag=0)
            if flag == 0:
                return flag
            back_scan_data = get_measure_insert_dict(
                measure_id, model_id, "back", back_scan_results, weight_constant)
            # print(back_scan_data)
            encoded_bcd = str(json.dumps(back_scan_data))
            flag = upload_message(
                queue_service, queue_name, encoded_bcd, flag=0)
            if flag == 0:
                return flag
            threesixty_scan_data = get_measure_insert_dict(
                measure_id, model_id, "360", threesixty_scan_results, weight_constant)
            # print(threesixty_scan_data)
            encoded_tcd = str(json.dumps(threesixty_scan_data))
            flag = upload_message(
                queue_service, queue_name, encoded_tcd, flag=0)
