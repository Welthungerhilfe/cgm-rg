...
class DatabaseInterface:
    ...
    def connect(self):
        ...
        self.dictionary_cursor = self.connection.cursor(
    ...
    def batch_execute(self, artifact_df, table):
        ...
        result = psycopg2.extras.execute_batch(
            self.cursor, insert_stmt, artifact_df.values)
        self.connection.commit()
...
def create_insert_statement(
        ...
        use_quotes_for_values=True):
    if convert_values_to_string:
        values = [str(value) for value in values]
    if use_quotes_for_values:
...
def create_update_statement(
    ...
    sql_statement += ", ".join([" {} = {}".format(key, value)
                                for key, value in zip(keys, values) if key != id_value])
