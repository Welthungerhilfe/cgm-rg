...
class HeightFlowRGBD(HeightFlow):
    ...
    def process_rgbd(self):
        ...
        for artifact in self.artifacts:
            ...
            result_image_dict = next(iter(item for item in self.image_artifacts if item['order'] == img_id), None)
            if result_image_dict:
                image_input_path = self.get_input_path(self.scan_image_directory, result_image_dict['file'])
            else:
            ...
            rgbd_data = tf.image.resize(rgbd_data,(preprocessing.IMAGE_TARGET_HEIGHT, preprocessing.IMAGE_TARGET_WIDTH))
