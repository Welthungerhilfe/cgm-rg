...
class WeightFlow:	
    ...
    def artifact_level_result(self, predictions, generated_timestamp):	
        ...
        for artifact, prediction in zip(self.artifacts, predictions):	
            ...
            result = Bunch(dict(	
                ...
                data={'weight': str(prediction[0]), 'pos_pe': artifact['percentile']['99_percentile_neg_error'],
                    'neg_pe': artifact['percentile']['99_percentile_pos_error'], 'mae': artifact['percentile']['mae']} if bool(artifact['percentile']) else {'weight': str(prediction[0])},
            ))	
    ...
    def scan_level_result(self, predictions, generated_timestamp, start_time, pos_percentile_error_99, neg_percentile_error_99, mae_scan):	
